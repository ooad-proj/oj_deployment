...
void play(char*filename,char**argv,char**env,FILE *config_file,char*line,int java,int edit){
...
struct rlimit max_memory,max_cpu_time,output;
   ...
   timelimit=atoi(fgets(line,1000,config_file));
...
output.rlim_cur = output.rlim_max =102400; 
...
int main() {
   ...
   error_file=freopen("fail.txt","w",stderr);

   FILE *readError_file=fopen("fail.txt","r");
...
while(1){
   ...
   if(!edit){
    ...
    char *filename1="/usr/bin/python3.8";

    char *argv1[] = {"python3.8","-m","py_compile","Main.py",NULL};
    ...
    char *argv1[] = {"python3.8","__pycache__/Main.cpython-38.pyc",NULL};
  ...
  // gettimeofday(&start, NULL);
   ...
   if(child_pid==0){
    ...
    play(filename,argv,env,config_file,line,java,edit);    
